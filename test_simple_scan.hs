-- Simple test of directory scanning functionality
...
main = do
  ...
  unless isDir $ do
    ...
    putStrLn "Please run: mkdir -p " ++ testDir
    putStrLn "And create some .py and .go files for testing"
    return ()
  ...
  putStrLn "Directory scanning test completed!" the directory structure first with:"
    putStrLn "  mkdir " ++ testDir
    putStrLn "  echo 'print(\"Hello from test1.py\")' > " ++ testDir ++ "/test1.py"
    putStrLn "  echo 'print(\"Hello from test2.py\")' > " ++ testDir ++ "/test2.py"
    return ()
