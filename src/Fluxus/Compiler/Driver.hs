...
import Fluxus.Debug.Debugger (debugPause, runDebugger)
...
convertConfigToDriver config = CompilerConfig
  ...
  , ccEnableDebug = Config.ccEnableDebugInfo config  -- Tie debug mode to debug info flag/env
...
runCompiler config action = do
  ...
  when (ccEnableDebug config) $
    TIO.putStrLn $ "[INFO] [DRIVER] Debug mode enabled for compiler"
  when (ccEnableDebug config) $
    TIO.putStrLn $ "[INFO] [DRIVER] Debug breakpoints: " <> T.pack (show $ ccDebugBreakpoints config)
  when (ccEnableDebug config) $
    TIO.putStrLn $ "[INFO] [DRIVER] Step mode: " <> T.pack (show $ ccStepMode config)
...
processSourceFile inputFile = do
  ...
  logInfo $ "[DRIVER] === Starting compilation pipeline for: " <> T.pack inputFile <> " ==="
  logInfo $ "[DRIVER] Debug breakpoint: processSourceFile called with file: " <> T.pack inputFile

  setCurrentPhase "parsing"
  when (ccEnableDebug config) $ do
    debugLog $ "[DEBUG] Starting parse stage for file: " <> T.pack inputFile
    liftIO $ void $ runDebugger (debugPause "Paused before parsing")
  ...
  typedAst <- if ccEnableAnalysis config
    then do
      ...
      logInfo $ "[DRIVER] Starting type inference for: " <> T.pack inputFile
      typeInferenceStage ast
    else do
      logInfo $ "[DRIVER] Skipping type inference (disabled)"
      return ast
  ...
  optimizedAst <- if ccOptimizationLevel config > O0
    then do
      ...
      logInfo $ "[DRIVER] Starting optimization for: " <> T.pack inputFile
      optimizationStage typedAst
    else do
      logInfo $ "[DRIVER] Skipping optimization (O0)"
      return typedAst

  logInfo $ "[DRIVER] === Pipeline completed for: " <> T.pack inputFile <> " ==="
  return optimizedAst
...
compileFile inputFile = do
  ...
  logInfo $ "[DRIVER] Starting code generation for: " <> T.pack inputFile
  cppCode <- codeGenStageMain optimizedAst

  -- Choose output filename for generated C++
  let desiredCppOut = case ccOutputPath config of
        Just p | takeExtension p == ".cpp" -> p
        _ -> replaceExtension inputFile ".cpp"
  logInfo $ "[DRIVER] Writing C++ output to: " <> T.pack desiredCppOut
  -- Embed original source as a C++ comment header to aid debugging and tests
  srcContent <- liftIO $ TIO.readFile inputFile
  let header = T.unlines ["// Generated by HyperStatic/CXX Compiler"
                         ,"/* Original Source (" <> T.pack inputFile <> "):", srcContent, "*/", ""]
      finalCpp = header <> renderCppUnit cppCode
  liftIO $ TIO.writeFile desiredCppOut finalCpp
  logInfo $ "[DRIVER] Successfully wrote C++ file: " <> T.pack desiredCppOut
  addIntermediateFile desiredCppOut
  
  -- Stop early if user requested a .cpp output or stopAtCodegen is set
  if ccStopAtCodegen config || (ccOutputPath config == Just desiredCppOut && takeExtension desiredCppOut == ".cpp")
    then do
      logInfo $ "Code generation completed: " <> T.pack desiredCppOut
      incrementProcessedFiles
      return desiredCppOut
    else do
      -- Continue with native compilation
      setCurrentPhase "c++-compilation"
      objFile <- compileCpp desiredCppOut
...
compileFileToObject inputFile isMain = do
  ...
  let header = T.unlines ["// Generated by HyperStatic/CXX Compiler"
                         ,"/* Original Source (" <> T.pack inputFile <> "):", srcContent, "*/", ""]
      finalCpp = header <> renderCppUnit cppCode
  liftIO $ TIO.writeFile cppFile finalCpp
...
parseStage inputFile = do
  ...
  logInfo $ "[DRIVER] Parse stage started for: " <> T.pack inputFile
  ...
  case detectedLanguage of
    Python -> do
      ...
      logInfo $ "[DRIVER] Python lexer starting for: " <> T.pack inputFile
      tokens <- case runPythonLexer (T.pack inputFile) content of
        Left err -> do
          logError $ "[DRIVER] Python lexer failed: " <> T.pack (show err)
        ...
        Right toks -> do
          ...
          logInfo $ "[DRIVER] Python lexer succeeded: " <> T.pack (show (length toks)) <> " tokens"
          return toks

      debugLog "Running Python parser"
      logInfo $ "[DRIVER] Python parser starting"
    ...
    Go -> do
      debugLog "Running Go lexer"
      logInfo $ "[DRIVER] Go lexer starting for: " <> T.pack inputFile
      tokens <- case runGoLexer (T.pack inputFile) content of
        Left err -> do
          logError $ "[DRIVER] Go lexer failed: " <> T.pack (show err)
          let (line, col) = extractPosFromError err
              srcSpan = SourceSpan (T.pack inputFile) (SourcePos line col) (SourcePos line col)
          throwError $ ParseError (T.pack $ show err) srcSpan
        Right toks -> do
          debugLog $ "Go lexer succeeded with " <> T.pack (show (length toks)) <> " tokens"
          logInfo $ "[DRIVER] Go lexer succeeded: " <> T.pack (show (length toks)) <> " tokens"
          return toks

      debugLog "Running Go parser"
      logInfo $ "[DRIVER] Go parser starting"
      case runGoParser (T.pack inputFile) tokens of
        Left err -> do
          logError $ "[DRIVER] Go parser failed: " <> T.pack (show err)
          let (line, col) = extractPosFromError err
              srcSpan = SourceSpan (T.pack inputFile) (SourcePos line col) (SourcePos line col)
          throwError $ ParseError (T.pack $ show err) srcSpan
        Right ast -> do
          debugLog "Go parser succeeded"
          logInfo $ "[DRIVER] Go parser succeeded"
          return $ Right ast
  where
...
typeInferenceStage ast = do
  ...
  logInfo $ "[DRIVER] Type inference stage started"
    ...
    Right success -> do
      if success
        then do
          ...
          logInfo $ "[DRIVER] Type inference completed successfully"
        else do
          logWarning $ "[DRIVER] Type inference found potential issues"
          let astSpan = extractSpanFromAST ast
...
codeGenStage ast = do
  ...
  logInfo $ "[DRIVER] Code generation stage started"
  let cppConfig = buildCppGenConfig config False
  debugLog $ "[DRIVER] CPP config built: " <> T.pack (show $ cgcOptimizationLevel cppConfig)
  let (cppUnit, _warnings) = generateCpp cppConfig ast
  logInfo $ "[DRIVER] Code generation completed successfully"
  return cppUnit

codeGenStageMain :: Either PythonAST GoAST -> CompilerM CppUnit
codeGenStageMain ast = do
  config <- ask
  debugLog "Starting main code generation"
  logInfo $ "[DRIVER] Main code generation stage started"
  let cppConfig = buildCppGenConfig config True
  debugLog $ "[DRIVER] Main CPP config built: " <> T.pack (show $ cgcOptimizationLevel cppConfig)
  let (cppUnit, _warnings) = generateCppMain cppConfig ast
  logInfo $ "[DRIVER] Main code generation completed successfully"
  return cppUnit
...
renderCppUnit (CppUnit { cppIncludes = includes, cppNamespaces = namespaces, cppDeclarations = decls }) =
  let (mainDecls, nonMainDecls) = partition isMainFunction decls
      isMainFunction (CppFunction "main" _ _ _) = True
      isMainFunction _ = False
  in T.unlines $
    -- Header intentionally omitted here; caller may prepend one with original source info
    map (\inc -> "#include " <> inc) includes ++
    map (\ns -> "namespace " <> ns <> " {") namespaces ++
    [ "" ] ++
    map renderCppDecl nonMainDecls ++
    map (\_ -> "}") namespaces ++
    [ "" ] ++  -- Empty line before main
    map renderCppDecl mainDecls
