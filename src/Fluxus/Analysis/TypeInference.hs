...
inferExpr (CECall func args) = do
  ...
  resultTypeVar <- freshTypeVar
  let expectedFuncType = TFunction argTypes resultTypeVar
...
applySubstitution subst = go
  where
    ...
    go (TForall vars constrs t) = TForall vars constrs (go t)
...
instantiate (TForall vars _ t) = do
  ...
  where
    ...
    extractTypeVar ty = error $ "Expected TVar but got: " <> show ty
