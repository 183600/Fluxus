...
combineBinaryShapes OpConcat left right
  | siIsHomogeneous left && siIsHomogeneous right && siElementType left == siElementType right = do
    return ShapeInfo
      { siDimensions = Vector.zipWith (+) (siDimensions left) (siDimensions right)
      , siIsKnown = siIsKnown left && siIsKnown right
      , siElementType = siElementType left
      , siFieldTypes = HashMap.empty
...
combineBinaryShapes _ left right = return unknownShape
