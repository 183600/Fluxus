...
constantFoldPythonExpr (Located span expr) = do
  newExpr <- case expr of
    ...
    PyDict pairs -> PyDict <$> mapM (\(k, v) -> (,) <$> constantFoldPythonExpr k <*> constantFoldPythonExpr v) pairs
...
collectLiveVarsPyExpr (Located _ expr) = case expr of
  ...
  PyDict pairs -> mapM_ (\(k, v) -> collectLiveVarsPyExpr k >> collectLiveVarsPyExpr v) pairs
  _ -> return ()
...
propagateConstantsPyExpr (Located span expr) = do
  newExpr <- case expr of
    ...
    PyDict pairs -> PyDict <$> mapM (\(k, v) -> (,) <$> propagateConstantsPyExpr k <*> propagateConstantsPyExpr v) pairs
...
simplifyAlgebraicPyExpr (Located span expr) = do
  newExpr <- case expr of
    ...
    PyDict pairs -> PyDict <$> mapM (\(k, v) -> (,) <$> simplifyAlgebraicPyExpr k <*> simplifyAlgebraicPyExpr v) pairs
...
strengthReducePyExpr (Located span expr) = do
  newExpr <- case expr of
    ...
    PyDict pairs -> PyDict <$> mapM (\(k, v) -> (,) <$> strengthReducePyExpr k <*> strengthReducePyExpr v) pairs
...
csePyExpr (Located span expr) = do
  newExpr <- case expr of
    ...
    PyDict pairs -> PyDict <$> mapM (\(k, v) -> (,) <$> csePyExpr k <*> csePyExpr v) pairs
