...
data PythonAST 
  = PyModule [PythonAST]
  | PyAssign Text PythonAST
  | PyBinaryOp BinaryOp PythonAST PythonAST
  | PyUnaryOp UnaryOp PythonAST
  | PyLiteral PyLiteral
  | PyIdentifier Text
  | PyFunctionDef Text [Text] [PythonAST]
  | PyIf PythonAST [PythonAST] [PythonAST]
  | PyReturn (Maybe PythonAST)
  | PyList [PythonAST]
  | PyDict [(PythonAST, PythonAST)]
  deriving (Show, Eq)
...
-- | Go AST types
data GoAST
  = GoPackage Text [GoAST]
  | GoFunction Text [(Text, Text)] (Maybe Text) [GoAST]
  | GoAssign Text GoAST
  | GoBinaryOp BinaryOp GoAST GoAST
  | GoUnaryOp UnaryOp GoAST
  | GoLiteral GoLiteral
  | GoIdentifier Text
  | GoIf GoAST [GoAST] [GoAST]
  | GoReturn (Maybe GoAST)
  | GoSlice [GoAST]
  | GoMap [(GoAST, GoAST)]
  deriving (Show, Eq)
...
foldPyBinaryOp op left right = fmap PyLiteral $ case op of
  ...
  BitwiseAnd -> case (left, right) of
    (PyInt a, PyInt b) -> Just $ PyInt (bitwiseAnd a b)
    _ -> Nothing
  
  BitwiseOr -> case (left, right) of
    (PyInt a, PyInt b) -> Just $ PyInt (bitwiseOr a b)
    _ -> Nothing
  
  BitwiseXor -> case (left, right) of
    (PyInt a, PyInt b) -> Just $ PyInt (bitwiseXor a b)
    _ -> Nothing
  
  LeftShift -> case (left, right) of
    (PyInt a, PyInt b) | b >= 0 && b < 64 -> Just $ PyInt (goShiftLeft a b)
    _ -> Nothing
  
  RightShift -> case (left, right) of
    (PyInt a, PyInt b) | b >= 0 && b < 64 -> Just $ PyInt (goShiftRight a b)
    _ -> Nothing
  
  where
    bitwiseAnd = goBitwiseAnd
    bitwiseOr = goBitwiseOr
    bitwiseXor = goBitwiseXor
...
foldPyUnaryOp op lit = fmap PyLiteral $ case op of
  ...
  BitwiseNot -> case lit of
    PyInt n -> Just $ PyInt (bitwiseNot n)
    _ -> Nothing
  
  where
    bitwiseNot = goBitwiseNot
...
goBitwiseAnd a b = a .&. b -- Bitwise AND

goBitwiseOr :: Integer -> Integer -> Integer
goBitwiseOr a b = a .|. b -- Bitwise OR

goBitwiseXor :: Integer -> Integer -> Integer
goBitwiseXor a b = a `xor` b -- Bitwise XOR
