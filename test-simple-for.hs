...
main = do
  content <- TIO.readFile "simple_for_test.py"
  case runPythonLexer (T.pack "simple_for_test.py") content of
    Left err -> putStrLn ("Lexer error: " ++ show err)
    Right tokens -> do
      putStrLn "Lexer success!"
      mapM_ printToken tokens
      case runPythonParser (T.pack "simple_for_test.py") tokens of
