...
struct __fluxus_math { static constexpr double pi = 3.141592653589793; } math;
...
int main() {
    ...
    if (len(sys::argv) != 3) {
        std::cout << to_str("Usage: ") << to_str(sys) << to_str(" <a> <b>") << std::endl;
        /* return (1; */
    }
    {
        {
            auto a = to_int(sys::argv[1]);
            auto b = to_int(sys::argv[2]);
            auto result = add(a, b);
            std::cout << result << std::endl;
        }
        if (__fluxus_exc) { auto e = 0;
            auto a = to_float(sys::argv[1]);
            auto b = to_float(sys::argv[2]);
            auto result = add(a, b);
            std::cout << result << std::endl;
        }
    }
