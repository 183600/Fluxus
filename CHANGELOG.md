...
- **[增强]** Python 列表和字符串的一维切片现在会通过 `fluxus_slice` 辅助函数生成 C++ 代码，避免简单切片触发运行时回退
...
- 改进了表达式生成的完整性
- 确保所有比较运算符都正确映射到C++
- Python 解析器现已支持 `async for` 与完整的切片/多维索引语法（如 `values[1:5:2]`、`matrix[:, idx, ...]`），并通过新的解析器单元测试予以覆盖，大幅提升了语法覆盖率。
