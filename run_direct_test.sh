#!/bin/bash

# Create a simple test program
cat > Test.hs << 'EOF'
{-# LANGUAGE OverloadedStrings #-}
module Main where

import Fluxus.Compiler.Driver
import System.Environment (getArgs)

main :: IO ()
main = do
  args <- getArgs
  let inputFile = if null args then "test_concurrent_fib.go" else head args
  
  putStrLn $ "Generating C++ from: " ++ inputFile
  
  let config = defaultConfig { 
        ccSourceLanguage = Go,
        ccStopAtCodegen = True,
        ccKeepIntermediates = True,
        ccVerboseLevel = 3
      }
  
  result <- runCompiler config $ do
    compileFile inputFile
  
  case result of
    Left err -> do
      putStrLn $ "Error: " ++ show err
    Right (_, state) -> do
      putStrLn $ "Successfully generated C++ source"
      putStrLn $ "Intermediate files: " ++ show (csIntermediateFiles state)
EOF

# Run with stack
echo "Running C++ generation test..."
stack runghc Test.hs test_concurrent_fib.go

# Check if C++ file was generated
if [ -f "test_concurrent_fib.cpp" ]; then
    echo "✓ C++ file generated: test_concurrent_fib.cpp"
    echo "File size: $(wc -l test_concurrent_fib.cpp) lines"
    echo ""
    echo "=== Generated C++ code preview ==="
    head -50 test_concurrent_fib.cpp
else
    echo "✗ C++ file not found"
fi