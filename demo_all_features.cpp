...
int calculate_sum(std::vector<int> items) {
    return sum(items);
}
...
int main() {
    ...
    auto matrix = std::vector{std::vector<decltype(1)>{1, 2}, std::vector<decltype(3)>{3, 4}};
    std::cout << "Matrix:" << " " << matrix << std::endl;
    auto squares = 0;
    std::cout << "Squares:" << " " << squares << std::endl;
    auto evens = 0;
    std::cout << "Evens:" << " " << evens << std::endl;
    auto result = calculate_sum(std::vector<decltype(10)>{10, 20, 30});
    ...
    std::cout << "Double of 5:" << " " << double_(5) << std::endl;
