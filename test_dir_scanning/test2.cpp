...
int main() {
    /* docstring */
    std::cout << "Advanced test file executed successfully!" << std::endl;
    auto fib_result = fibonacci(10);
    auto fact_result = factorial(5);
    std::cout << to_str("Fibonacci(10) = ") << to_str(fib_result) << std::endl;
    std::cout << to_str("Factorial(5) = ") << to_str(fact_result) << std::endl;
    ...
    auto word_counts = std::unordered_map<std::string, int>{{"hello", 3}, {"world", 2}, {"python", 5}};
    for (auto [word, count] : py_items(word_counts)) {
        std::cout << to_str("'") << to_str(word) << to_str("' appears ") << to_str(count) << to_str(" times") << std::endl;
    }
    std::cout << "All advanced functionality works!" << std::endl;
    return 0;
}
