...
class Person {
    public:
        ...
        std::string greet() {
    return to_str("Hi, I'm ") + to_str(this->name) + to_str(".");
}


        std::string celebrate_birthday() {
    this->age += 1;
    return to_str("Happy ") + to_str(this->age) + to_str("th birthday!");
}
...
int main() {
    ...
    /* docstring */
    auto int_var = 42;
    auto float_var = 3.14159;
    auto str_var = "Hello, World!";
    auto bool_var = true;
    auto list_var = std::vector<std::string>{to_str(1), to_str(2), to_str(3), "four", to_str(5.0)};
    auto tuple_var = (1, 2, 3, "four");
    auto dict_var = std::unordered_map<std::string, std::string>{{"name", "Alice"}, {"age", to_str(30)}, {"scores", to_str(std::vector<decltype(85)>{85})}, {"90", to_str(0)}, {"95]", to_str(0)}};
    auto set_var = {1, 2, 3, 3, 2, 1};
    if (int_var > 0) {
    }
    else if (int_var < 0) {
    }
    ...
    std::cout << "Simple test file executed successfully!" << std::endl;
    std::cout << greet("World") << std::endl;
    std::cout << to_str("2^3 = ") << to_str(power(2, 3)) << std::endl;
    std::cout << to_str("Sum of 1+2+3 = ") << to_str(sum_all(1, 2, 3)) << std::endl;
    auto person = Person("Alice", 30);
    std::cout << person.greet() << std::endl;
    std::cout << person.celebrate_birthday() << std::endl;
    std::cout << "All basic functionality works!" << std::endl;
    return 0;
}
