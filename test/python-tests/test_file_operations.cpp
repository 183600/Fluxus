...
int main() {
    auto test_file = "test_file.txt";
    {
    }
    std::cout << to_str("File '") << to_str(test_file) << to_str("' created and written to.") << std::endl;
    {
    }
    ...
    std::cout << "\nAfter appending:" << std::endl;
    {
    }
    std::cout << to_str("\\nFile exists: ") << to_str(os.path.exists(test_file)) << std::endl;
    std::cout << to_str("File size: ") << to_str(os.path.getsize(test_file)) << to_str(" bytes") << std::endl;
    std::cout << "\nDifferent read modes:" << std::endl;
    {
    }
    {
    }
    auto binary_file = "test_binary.bin";
    {
    }
    std::cout << to_str("\\nBinary file '") << to_str(binary_file) << to_str("' created.") << std::endl;
    {
    }
    os.remove(test_file);
    os.remove(binary_file);
    std::cout << to_str("\\nTest files '") << to_str(test_file) << to_str("' and '") << to_str(binary_file) << to_str("' removed.") << std::endl;
    return 0;
}
