...
class Timer {
    ...
    std::cout << to_str("Execution time: ") << ([&](){ std::ostringstream os; os.setf(std::ios::fixed); os<<std::setprecision(4); os<<this->end_time - this->start_time; return os.str(); }()) << to_str(" seconds") << std::endl;
}
...
void read_file_example() {
    ...
    {
        std::cout << "\nReading line by line:" << std::endl;
        for (size_t i = 0; i < f, 1.size(); ++i) {
        auto line = f, 1[i];
            std::cout << to_str("Line ") << to_str(i) << to_str(": ") << to_str(line.strip()) << std::endl;
        }
    }
    os.remove(filename);
}
...
void copy_file_example() {
    auto source = "source.txt";
    auto dest = "destination.txt";
    {
...
void test_timer() {
    std::cout << "\nTesting custom context manager:" << std::endl;
    {
        time.sleep(0.1);
