...
class Person {
    ...
    return to_str("My name is ") + to_str(this->name) + to_str(" and I am ") + to_str(this->age) + to_str(" years old.");
}


        std::string celebrate_birthday() {
    this->age += 1;
    return to_str("Happy birthday! I'm now ") + to_str(this->age) + to_str(" years old.");
}
...
class Student : public Person {
    public:
        Student(std::string name, int age, std::string student_id) : Person(name, age) {
            this->student_id = student_id;
            this->grades = std::vector<int>{};
        }
    ...
    return (!this->grades.empty() ? sum(this->grades) / len(this->grades) : 0);
}


        std::string introduce() {
    return to_str("My name is ") + to_str(this->name) + to_str(", I'm ") + to_str(this->age) + to_str(" years old, and my student ID is ") + to_str(this->student_id) + to_str(".");
}
...
int main() {
    ...
    auto student1 = Student("Bob", 20, "S12345");
    std::cout << person1.introduce() << std::endl;
    std::cout << person1.celebrate_birthday() << std::endl;
    std::cout << student1.introduce() << std::endl;
    student1.add_grade(90);
    student1.add_grade(85);
    student1.add_grade(95);
    std::cout << to_str("Student's average grade: ") << ([&](){ std::ostringstream os; os.setf(std::ios::fixed); os<<std::setprecision(2); os<<student1.get_average_grade(); return os.str(); }()) << std::endl;
    return 0;
}
