...
class FileManager {
    ...
    std::cout << to_str("Opening file: ") << to_str(this->filename) << std::endl;
    this->file = to_str("file_handle_") + to_str(this->filename);
    return this->file;
}


        bool __exit__(int exc_type, int exc_val, int exc_tb) {
    std::cout << to_str("Closing file: ") << to_str(this->filename) << std::endl;
    if (exc_type) {
        std::cout << to_str("Exception occurred: ") << to_str(exc_type) << std::endl;
    }
    return false;
}
