...
void test_list_operations() {
    ...
    auto assert len(numbers) = = 5;
    auto assert numbers[0] = = 1;
    auto assert numbers[-1] = = 5;
    auto assert sum(numbers) = = 15;
    auto assert sorted(numbers) = = numbers;
}


void test_dictionary_operations() {
    auto data = std::unordered_map<std::string, std::string>{{"name", "Alice"}, {"age", to_str(25)}, {"city", "New York"}};
    auto assert data["name"] = = "Alice";
    assert data.get("age")(= 25);
    assert "city" in data;
    auto assert len(data) = = 3;
}
...
int main() {
    ...
    test_boolean_logic();
    std::cout << "All Python basics tests passed!" << std::endl;
    return 0;
}
