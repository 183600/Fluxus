...
int main() {
    ...
    auto diff = y - x;
    auto product = x * y;
    auto quotient = to_float(y) / to_float((x));
    auto remainder = y % x;
    ...
    std::cout << "Last:" << " " << numbers[4] << std::endl;
    auto person = std::unordered_map<std::string, std::string>{{"name", "Bob"}, {"age", to_str(30)}};
    std::cout << "Person:" << " " << person << std::endl;
    std::cout << "Name:" << " " << person["name"] << std::endl;
    for (int i = 0; i < 3; ++i) {
    }
    auto result = add_numbers(5, 3);
    std::cout << "add_numbers(5, 3):" << " " << result << std::endl;
